...
export default class ClassVisManager extends VisManager {
    ...
    get_array() {
        ...
        let propMask = this.node.propertyMask;
        let mask = false;
        if (typeof(propMask) !== 'undefined') {
            mask = true;
        }
        for (let field of this.node.fields.values()) {
            if (mask) {
                if (propMask.has(field.name)) rowsArray.push(field);
            }
            else {
        ...
        if (! this.open) {
            rowsArray = rowsArray.filter(d => !(
                d.rootKind !== 'Header' &&
                d.rootKind !== 'OBJECT' &&
                d.rootKind !== 'INTERFACE' &&
                d.rootKind !== 'UNION' &&
                d.rootKind !== 'ENUM'))
        }
        rowsArray.sort((a, b) => {
